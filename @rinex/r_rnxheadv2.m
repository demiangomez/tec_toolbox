    ...
    switch tag
        ...
        case '# / TYPES OF OBSERV'
            ...
            if ~isempty(strfind(temp,'C2'))
                ...
                inf.seq.glo(6) = (strfind(temp,'C2')+1)/2;
            end
            % P1
...
if isempty(inf.time.leap)
    [~,mjd] = rinex.cal2jul(inf.time.first(1),inf.time.first(2),inf.time.first(3),...
        (inf.time.first(4)*3600 + inf.time.first(5)*60 + inf.time.first(6)));
    
    inf.time.leap = rinex.leapsec(mjd);
    if strcmp(inf.time.system,'GPS')
        inf.time.leap = inf.time.leap - 19;
    end
end

% DDG: determine the sampling interval from the data if not present in the
% header
if isnan(inf.time.int)
    tline   = fgetl(fid);
    ep1     = sscanf(tline(1:end),'%f',[1,8]);
    sats_no = ep1(8);
    ...
    lines = lines + ceil(inf.obsno/5)*sats_no - 1;
    for i = 1:(lines+1)
        tline = fgetl(fid);
    end
    % read the second epoch
    ep2 = sscanf(tline(1:end),'%f',[1,8]);
    inf.time.int = seconds(datetime(ep2(1:6)) - datetime(ep1(1:6)));
end

[doy] = rinex.clc_doy(inf.time.first(1),inf.time.first(2),inf.time.first(3));
inf.time.doy = doy;
