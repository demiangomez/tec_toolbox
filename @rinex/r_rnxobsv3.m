...
% function to read multiconstellation RINEX 3 files
%
    ...
    if ~isempty(tline) && strcmp(tline(1),'>') 
        ...
        for i=1:nep(8)
            ...
            if strcmp(tline(1),'G') && (options.system.gps == 1)
                ...
                l1s(epno,k) = lso(inf.seq.gps(3));
                % l2s(epno,k) = lso(inf.seq.gps(4))*wavl(k,2);
                l2s(epno,k) = lso(inf.seq.gps(4));
            % GLONASS
            elseif strcmp(tline(1),'R') && (options.system.glo == 1)
                k   = rinex.max_sv_index('gps') + sscanf(tline(2:3),'%d'); % satellite prn number
                if k>rinex.max_sv_index('glonass')
                    continue
                end
                ...
                l1s(epno,k) = lso(inf.seq.glo(3));
                % l2s(epno,k) = lso(inf.seq.glo(4))*wavl(k,2);
                l2s(epno,k) = lso(inf.seq.glo(4));
            % GALILEO
            elseif strcmp(tline(1),'E') && (options.system.gal == 1)
                k   = rinex.max_sv_index('glonass') + sscanf(tline(2:3),'%d'); % satellite prn number
                if k>rinex.max_sv_index('galileo')
                    continue
                end
                ...
                l1s(epno,k) = lso(inf.seq.gal(3));
                % l2s(epno,k) = lso(inf.seq.gal(4))*wavl(k,2);
                l2s(epno,k) = lso(inf.seq.gal(4));
            % BEIDOU
            elseif strcmp(tline(1),'C') && (options.system.bds == 1)
                k   = rinex.max_sv_index('galileo') + sscanf(tline(2:3),'%d'); % satellite prn number
                if k>rinex.max_sv_index('beidou')
                    continue
                end
                ...
                l1s(epno,k) = lso(inf.seq.bds(3));
                % l2s(epno,k) = lso(inf.seq.bds(4))*wavl(k,2);
                l2s(epno,k) = lso(inf.seq.bds(4));
            end
        end
    end
end
